...
-> 성화대가 위로 솟아오른다 -> 불이 활활 타오른다 -> 문구가 뜬다 -> 끝

- 길이 : 약 11초. 건너뛰기 버튼 있음, 아무 데나 눌러도 넘어감, Esc/Space/Enter 도 됨
...
| `sub` | `'우리들의 운동회, 지금 시작!'` | 둘째 줄 |
| `tag` | `'어디에 있든 우리는 같은 운동장'` | 셋째 줄 |
| `onDone` | `null` | 끝나거나 건너뛰었을 때 호출. 여기서 원래 화면을 띄운다 |
...
- 큰 불꽃이 타오르고 그 아래에 제목 세 줄이 뜬다
- 약 11초 뒤 저절로 사라지고 `onDone` 이 불린다
- 건너뛰기 버튼을 누르면 즉시 `onDone` 이 불린다
